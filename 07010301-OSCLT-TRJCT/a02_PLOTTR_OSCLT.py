...
rad_p        = np.sqrt(np.array([10., 1., 0.1, 0.01])) * \
...
for j in range(0, len(S_v)):
    ...
    particle_nohistory_dic[j] = maxey_riley_oscillatory_noHistory(j+1,
                                            y0, v0,
                                            tini, vel,
                                            particle_density    = rho_p,
                                            fluid_density       = rho_f,
...
for j in range(0, len(S_v)):
    ...
    plt.plot(particle_nohistory_dic[j].pos_vec_nohistory[:,0] * L_scale,
             particle_nohistory_dic[j].pos_vec_nohistory[:,1] * L_scale,
    ...
    filename = code + '-OSCLT-TRJCT-R=' + str(round(particle_dic[j].p.R, 2)) +\
