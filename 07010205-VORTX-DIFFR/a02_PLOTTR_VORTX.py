...
rho_p        = np.array([1./3., 2./3., 4./3., 2.])    # - Particle's density
...
rad_p        = np.sqrt(np.geomspace(100., 0.1, 101)) * \
...
for j in progressbar(range(0, len(S_v))):
    ...
    particle1_dic[j]  = maxey_riley_analytic(j+1, y0, v0, tini, vel,
    ...
    particle1_nohistory_dic[j] = maxey_riley_analytic_nohistory(j+1, y0, v0,
                                            tini, vel,
                                            particle_density    = rho_p[0],
                                            fluid_density       = rho_f,
                                            particle_radius     = rad_p[j],
    ...
    # Create particle class instances
    particle2_dic[j]  = maxey_riley_analytic(j+1, y0, v0, tini, vel,
    ...
    particle2_nohistory_dic[j] = maxey_riley_analytic_nohistory(j+1, y0, v0,
                                            tini, vel,
                                            particle_density    = rho_p[1],
                                            fluid_density       = rho_f,
                                            particle_radius     = rad_p[j],
    ...
    # Create particle class instances
    particle3_dic[j]  = maxey_riley_analytic(j+1, y0, v0, tini, vel,
    ...
    particle3_nohistory_dic[j] = maxey_riley_analytic_nohistory(j+1, y0, v0,
                                            tini, vel,
                                            particle_density    = rho_p[2],
                                            fluid_density       = rho_f,
                                            particle_radius     = rad_p[j],
    ...
    # Create particle class instances
    particle4_dic[j]  = maxey_riley_analytic(j+1, y0, v0, tini, vel,
    ...
    particle4_nohistory_dic[j] = maxey_riley_analytic_nohistory(j+1, y0, v0,
                                            tini, vel,
                                            particle_density    = rho_p[3],
                                            fluid_density       = rho_f,
                                            particle_radius     = rad_p[j],
    ...
    traj1_diff = np.append(traj1_diff, max(diff1 / np.linalg.norm(particle1_dic[j].pos_vec - particle1_dic[j].pos_vec[0], axis=1)))
    
    diff2 = np.linalg.norm(particle2_dic[j].pos_vec - particle2_nohistory_dic[j].pos_vec, axis=1)
    traj2_diff = np.append(traj2_diff, max(diff2 / np.linalg.norm(particle2_dic[j].pos_vec - particle2_dic[j].pos_vec[0], axis=1)))
    
    diff3 = np.linalg.norm(particle3_dic[j].pos_vec - particle3_nohistory_dic[j].pos_vec, axis=1)
    traj3_diff = np.append(traj3_diff, max(diff3 / np.linalg.norm(particle3_dic[j].pos_vec - particle3_dic[j].pos_vec[0], axis=1)))
    
    diff4 = np.linalg.norm(particle4_dic[j].pos_vec - particle4_nohistory_dic[j].pos_vec, axis=1)
    traj4_diff = np.append(traj4_diff, max(diff4 / np.linalg.norm(particle4_dic[j].pos_vec - particle4_dic[j].pos_vec[0], axis=1)))
...
filename = '07010205-VORTX-DIFFR.pdf'
