...
c_v          = np.array([1., 2., 5., 10., 15., 20., 30., 50., 100., 200.])
...
for j in range(0, len(c_v[1:])):    
    ...
    xi_fd_v     = np.linspace(0., 1., int(2*N))[:-1]

    # Logarithm map to obtain QUM
    x_fd_v      = -c_v[0] * np.log(1.0 - xi_fd_v)

    Analytic_particle  = maxey_riley_imex(1, y0, v0, vel,
                                        x_fd_v, c_v[0], dt, tini,
...
plt.plot(c_v[1:], IMEX2_err_v,   's-', color='violet', label="FD2 + IMEX2",           linewidth=lw, markersize=ms)
plt.plot(c_v[1:], Trap_err_v,    'v-', color='green',  label="FD2 + Trap. Rule",      linewidth=lw, markersize=ms)
plt.plot(c_v[1:], IMEX4_err_v,   'o-', color='red',    label="FD4 + IMEX4",           linewidth=lw, markersize=ms)
plt.plot(c_v[1:], DIRK4_err_v,   '+-', color='blue',   label="FD4 + DIRK4",           linewidth=lw, markersize=ms)

plt.xscale("log")
plt.yscale("log")
plt.ylim(2e-10,3e-3) # plt.ylim(7e-3,1e-2)
...
plt.savefig(save_plot_to + '06020107-DGYRE-CNVRG-C-NZROQ.pdf', format='pdf', dpi=500)

call(["pdfcrop", save_plot_to + '06020107-DGYRE-CNVRG-C-NZROQ.pdf', save_plot_to + '06020107-DGYRE-CNVRG-C-NZROQ.pdf'])
