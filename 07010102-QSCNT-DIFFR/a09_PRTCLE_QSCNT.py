...
class maxey_riley_relaxing(object):
  ...
  def __init__(self, tag, x, v, tini,
      ...
      self.vel_vec_nohistory = np.copy(v)
  ...
  def solve_nohistory(self, t):
      ...
      # Calculate velocity
      # Add initial velocity
      expfun     = np.exp(-(t - self.tini) / (self.p.R * self.p.S))
      
      vxresult   = self.v[0] * expfun
      vyresult   = self.v[1] * expfun
      
      V_vec      = np.array([vxresult, vyresult])
      self.vel_vec_nohistory = np.vstack((self.vel_vec_nohistory, V_vec))
      ...
      xresult   += self.p.R * self.p.S * (self.v[0] - vxresult)
      yresult   += self.p.R * self.p.S * (self.v[1] - vyresult)
