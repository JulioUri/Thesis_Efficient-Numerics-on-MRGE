...
for k in range(0, len(S_v)):
    ...
    plt.plot(particle_dic[k].pos_vec[:,0] * L_scale,
             particle_dic[k].pos_vec[:,1] * L_scale,
             color='red', linewidth=lw, label="with History Term")
    plt.plot(particle_nohistory_dic[k].pos_vec[:,0] * L_scale,
             particle_nohistory_dic[k].pos_vec[:,1] * L_scale,
    ...
    filename = code + '-DGYRE-TRJCT-R=' + str(round(particle_dic[k].p.R, 2)) +\
