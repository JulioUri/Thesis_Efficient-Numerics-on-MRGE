...
rho_p        = np.array([2./3., 1., 3./2.]) # - Particle's density
...
rad_p        = np.sqrt(0.3) * np.sqrt(3. * nu_f * T_scale)  # Particle's radius
...
for j in range(0, len(rho_p)):
    ...
    if j == 0:
        filename = '06010201-VORTX-TRJCTa.pdf'
    elif j == 1:
        filename = '06010201-VORTX-TRJCTb.pdf'
    elif j == 2:
        filename = '06010201-VORTX-TRJCTc.pdf'
