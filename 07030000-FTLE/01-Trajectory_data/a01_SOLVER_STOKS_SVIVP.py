...
from a09_PRTCLE_STOKS_SVIVP import particle_stokes
...
if exists(save_output_to + "b02_STKS2_TIMEVC-R=" + str(R) + "-St=" + str(St) + ".hdf5"):
    os.remove(save_output_to + "b02_STKS2_TIMEVC-R=" + str(R) + "-St=" + str(St) + ".hdf5")

if exists(save_output_to + "b02_STKS2_TRAJCT-R=" + str(R) + "-St=" + str(St) + ".hdf5"):
    os.remove(save_output_to + "b02_STKS2_TRAJCT-R=" + str(R) + "-St=" + str(St) + ".hdf5")

if exists(save_output_to + "b02_STKS2_VELOCT-R=" + str(R) + "-St=" + str(St) + ".hdf5"):
    os.remove(save_output_to + "b02_STKS2_VELOCT-R=" + str(R) + "-St=" + str(St) + ".hdf5")
...
def save_hdf5(vector):
    with h5py.File(save_output_to + 'b02_STKS2_TRAJCT-R=' + str(R) + '-St=' + str(St) + '.hdf5', 'a') as f:
        f.create_dataset(str(vector[0]), data=vector[1])
        
    with h5py.File(save_output_to + 'b02_STKS2_VELOCT-R=' + str(R) + '-St=' + str(St) + '.hdf5', 'a') as f:
...
# Save times
with h5py.File(save_output_to + 'b02_STKS2_TIMEVC-R=' + str(R) + '-St=' + str(St) + '.hdf5', 'w') as f:
    f.create_dataset("Parameters", data=str(parameter_dic))
...
if parallel_flag == False:
    with h5py.File(save_output_to + 'b02_STKS2_TRAJCT-R=' + str(R) + '-St=' + str(St) + '.hdf5', 'w') as f:
        f.create_dataset("Parameters", data=str(parameter_dic))
        for particle in Stokes_particle_set:
            f.create_dataset(str(particle.tag), data=particle.pos_vec)
        
    with h5py.File(save_output_to + 'b02_STKS2_VELOCT-R=' + str(R) + '-St=' + str(St) + '.hdf5', 'w') as f:
        f.create_dataset("Parameters", data=str(parameter_dic))
        for particle in Stokes_particle_set:
