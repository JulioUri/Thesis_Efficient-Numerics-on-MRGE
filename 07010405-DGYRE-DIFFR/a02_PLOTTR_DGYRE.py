...
y0           = np.array([.5, 0.6]) / L_scale  # Initial position (nondimensional)
...
for j in progressbar(range(0, len(S_v))):
    ...
    traj1_diff = np.append(traj1_diff, max(diff1[1:]) / max(np.linalg.norm(particle1_dic[j].pos_vec[1:] - particle1_dic[j].pos_vec[0], axis=1)))
    
    diff2 = np.linalg.norm(particle2_dic[j].pos_vec - particle2_nohistory_dic[j].pos_vec, axis=1)
    traj2_diff = np.append(traj2_diff, max(diff2[1:]) / max(np.linalg.norm(particle2_dic[j].pos_vec[1:] - particle2_dic[j].pos_vec[0], axis=1)))
    
    diff3 = np.linalg.norm(particle3_dic[j].pos_vec - particle3_nohistory_dic[j].pos_vec, axis=1)
    traj3_diff = np.append(traj3_diff, max(diff3[1:]) / max(np.linalg.norm(particle3_dic[j].pos_vec[1:] - particle3_dic[j].pos_vec[0], axis=1)))
    
    diff4 = np.linalg.norm(particle4_dic[j].pos_vec - particle4_nohistory_dic[j].pos_vec, axis=1)
    traj4_diff = np.append(traj4_diff, max(diff4[1:]) / max(np.linalg.norm(particle4_dic[j].pos_vec[1:] - particle4_dic[j].pos_vec[0], axis=1)))
...
filename = '07010405-DGYRE-DIFFR.pdf'
